...
int main()
{
    ...
    rtn = MPIN_GET_CLIENT_PERMIT(HASH_TYPE_MPIN,yesterday,&MS1,&HCID,&TP1);
    if (rtn != 0)
    {
        printf("MPIN_GET_CLIENT_PERMIT(yesterday,&MS1,&HCID,&TP1) Error %d\n", rtn);
        return 1;
    }
    rtn = MPIN_GET_CLIENT_PERMIT(HASH_TYPE_MPIN,yesterday,&MS2,&HCID,&TP2);
    if (rtn != 0)
    {
    ...
    /* Server second pass */
#ifdef USE_DVS
    rtn = MPIN_SERVER_2(date,&HID,&HTID,&Y,&ServerSecret,&U,&UT,&SEC,&E,&F,NULL);
#else
