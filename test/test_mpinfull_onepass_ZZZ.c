/**
 * @file test_mpinfull_onePAS_ZZZs_ZZZ.c
 * @author Kealan McCusker
 * @brief Test M-Pin Full Single PAS_ZZZs
 *
 * LICENSE
 *
...
/* Test M-Pin Full Single PAS_ZZZs */
...
int main()
{
    ...
    /* Server  */
#ifdef USE_DVS
    rtn = MPIN_ZZZ_SERVER(HASH_TYPE_MPIN_ZZZ,date,&HID,&HTID,&Y2,&ServerSecret,NULL,&UT,&SEC,&E,&F,pID,NULL,TimeValue,NULL);
#else
    rtn = MPIN_ZZZ_SERVER(HASH_TYPE_MPIN_ZZZ,date,&HID,&HTID,&Y2,&ServerSecret,NULL,&UT,&SEC,&E,&F,pID,NULL,TimeValue);
#endif
    ...
    HASH_ALL(HASH_TYPE_MPIN_ZZZ,&HCID,NULL,&UT,&SEC,&Y1,&Z,&T,&HM);
    MPIN_ZZZ_CLIENT_KEY(HASH_TYPE_MPIN_ZZZ,&G1,&G2,PIN2,&R,&X,&HM,&T,&CK);
    printf("Client Key = ");
    OCT_output(&CK);

    HASH_ALL(HASH_TYPE_MPIN_ZZZ,&HCID,NULL,&UT,&SEC,&Y2,&Z,&T,&HM);
