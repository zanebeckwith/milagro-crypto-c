...
if __name__ == "__main__":
    ...
    if ONE_PASS:
        ...
        # Client MPIN
        time_func(
            'rtn, x, u, ut, v, y = mpin_ZZZ.client(HASH_TYPE_MPIN, date, mpin_id, rng, None, PIN, token, time_permit, None, epoch_time)',
            nIter)
        ...
        # Client first pass
        time_func(
            'rtn, x, u, ut, sec = mpin_ZZZ.client_1(HASH_TYPE_MPIN, date, mpin_id, rng, None, PIN, token, time_permit)',
            nIter)
        ...
        if MPIN_FULL:
            ...
            time_func(
                'rtn, server_aes_key = mpin_ZZZ.server_key(HASH_TYPE_MPIN, Z, server_secret, w, HM, HID, u, ut)',
                nIter)
            rtn, server_aes_key = mpin_ZZZ.server_key(
                HASH_TYPE_MPIN, Z, server_secret, w, HM, HID, u, ut)
            if rtn != 0:
