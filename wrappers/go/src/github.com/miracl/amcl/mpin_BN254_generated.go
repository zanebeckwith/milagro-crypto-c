...
func RandomGenerate_BN254(rng *Rand) ([]byte, error) {
	r := make([]byte, PGS_BN254)
	rtn := C._MPIN_BN254_RANDOM_GENERATE(rng.csprng(), *makeOctet(r))
	return r, newError(int(rtn))
}

// GetServerSecret_BN254 creates a server secret in G2 from a master secret
func GetServerSecret_BN254(ms []byte) ([]byte, error) {
	ss := make([]byte, G2S_BN254)
	rtn := C._MPIN_BN254_GET_SERVER_SECRET(*newOctet(ms), *makeOctet(ss))
...
func RecombineG1_BN254(r1, r2 []byte) ([]byte, error) {
	r := make([]byte, G1S_BN254)
	rtn := C._MPIN_BN254_RECOMBINE_G1(*newOctet(r1), *newOctet(r2), *makeOctet(r))
	return r, newError(int(rtn))
}

// RecombineG2_BN254 adds two members from the group G2
func RecombineG2_BN254(w1, w2 []byte) ([]byte, error) {
	w := make([]byte, G2S_BN254)
	rtn := C._MPIN_BN254_RECOMBINE_G2(*newOctet(w1), *newOctet(w2), *makeOctet(w))
...
func GetClientSecret_BN254(masterSecret []byte, hashMPinId []byte) ([]byte, error ) {
	...
	rtn := C._MPIN_BN254_GET_CLIENT_SECRET(*newOctet(masterSecret), *newOctet(hashMPinId), *makeOctet(cs))
...
func GetDVSKeyPair_BN254(rng *Rand, z []byte) (zOut, publicKey []byte, err error) {
	publicKey = make([]byte, G2S_BN254)

	var rtn C.int
	if rng != nil {
		z = make([]byte, PGS_BN254)
		rtn = C._MPIN_BN254_GET_DVS_KEYPAIR(rng.csprng(), *makeOctet(z), *makeOctet(publicKey))
	} else {
		rtn = C._MPIN_BN254_GET_DVS_KEYPAIR(nil, *newOctet(z), *makeOctet(publicKey))
	}
...
func GetClientPermit_BN254(hashType, epochDate int, masterSecret, hashMPinId []byte) ([]byte, error) {
	tp := make([]byte, G1S_BN254)
	rtn := C._MPIN_BN254_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), *newOctet(masterSecret), *newOctet(hashMPinId), *makeOctet(tp))
...
func ExtractPIN_BN254(hashType int, mpinId []byte, PIN int, clientSecret []byte) ([]byte, error) {
	rtn := C._MPIN_BN254_EXTRACT_PIN(C.int(hashType), *newOctet(mpinId), C.int(PIN), *newOctet(clientSecret))
...
func Precompute_BN254(token, hashMPinId []byte) (pc1, pc2 []byte, err error) {
	pc1, pc2 = make([]byte, GTS_BN254), make([]byte, GTS_BN254)
	rtn := C._MPIN_BN254_PRECOMPUTE(*newOctet(token), *newOctet(hashMPinId), nil, *makeOctet(pc1), *makeOctet(pc2))
...
// Returns:
//     V: V = -(x+y)(CS+TP), where CS is the reconstructed client secret and TP is the time permit
func Client2_BN254(x, y, SEC []byte) ([]byte, error) {
	rtn := C._MPIN_BN254_CLIENT_2(*newOctet(x), *newOctet(y), *newOctet(SEC))
