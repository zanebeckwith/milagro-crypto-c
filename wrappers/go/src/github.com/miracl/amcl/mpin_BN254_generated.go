...
// ECC points constant
const PAS_BN254 int = int(C.MPIN_PAS)
const PGS_BN254 int = int(C.MPIN_PGS_BN254)
const PFS_BN254 int = int(C.MPIN_PFS_BN254)
const G1S_BN254 = 2*PFS_BN254 + 1
const G2S_BN254 = 4 * PFS_BN254
const GTS_BN254 = 12 * PFS_BN254
...
func RandomGenerate_BN254(RNG *Rand) (errorCode int, S []byte) {
	...
	rtn := C.MPIN_BN254_RANDOM_GENERATE(RNG.csprng(), &SOct)
	errorCode = int(rtn)
...
GetServerSecret_BN254 creates a server secret in G2 from a master secret
...
*/
func GetServerSecret_BN254(masterSecret []byte) (errorCode int, serverSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	serverSecretOct := GetOctetZero(G2S_BN254)
	defer OctetFree(&serverSecretOct)

	rtn := C.MPIN_BN254_GET_SERVER_SECRET(&masterSecretOct, &serverSecretOct)
	errorCode = int(rtn)
...
*/
func RecombineG1_BN254(R1 []byte, R2 []byte) (errorCode int, R []byte) {
	// Form Octets
	R1Str := string(R1)
	...
	ROct := GetOctetZero(G1S_BN254)
	defer OctetFree(&ROct)

	rtn := C.MPIN_BN254_RECOMBINE_G1(&R1Oct, &R2Oct, &ROct)
	errorCode = int(rtn)
...
RecombineG2_BN254 adds two members from the group G2
...
*/
func RecombineG2_BN254(W1 []byte, W2 []byte) (errorCode int, W []byte) {
	// Form Octets
	W1Str := string(W1)
	...
	WOct := GetOctetZero(G2S_BN254)
	defer OctetFree(&WOct)

	rtn := C.MPIN_BN254_RECOMBINE_G2(&W1Oct, &W2Oct, &WOct)
	errorCode = int(rtn)
...
*/
func GetClientSecret_BN254(masterSecret []byte, hashMPinId []byte) (errorCode int, clientSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	...
	clientSecretOct := GetOctetZero(G1S_BN254)
	defer OctetFree(&clientSecretOct)

	rtn := C.MPIN_BN254_GET_CLIENT_SECRET(&masterSecretOct, &hashMPinIdOct, &clientSecretOct)
	errorCode = int(rtn)
...
GetDVSKeyPair_BN254 randomly generates the public key in G2 for the key-escrow less scheme
...
func GetDVSKeyPair_BN254(RNG *Rand, z []byte) (errorCode int, zOut []byte, publicKey []byte) {
	...
	if RNG == nil {
		...
		pRNG = RNG.csprng()
		zOct = GetOctetZero(PGS_BN254)
	}

	rtn := C.MPIN_BN254_GET_DVS_KEYPAIR(pRNG, &zOct, &publicKeyOct)
	errorCode = int(rtn)
...
GetClientPermit_BN254 creates a time permit in G1 from a master secret, hash of the M-Pin Id and epoch days
...
*/
func GetClientPermit_BN254(hashType, epochDate int, masterSecret, hashMPinId []byte) (errorCode int, timePermit []byte) {
	// Form Octets
	...
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
	timePermitOct := GetOctetZero(G1S_BN254)
	defer OctetFree(&timePermitOct)

	rtn := C.MPIN_BN254_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), &masterSecretOct, &hashMPinIdOct, &timePermitOct)
	errorCode = int(rtn)
...
*/
func ExtractPIN_BN254(hashType int, mpinId []byte, PIN int, clientSecret []byte) (errorCode int, token []byte) {
	// Form Octets
	...
	rtn := C.MPIN_BN254_EXTRACT_PIN(C.int(hashType), &mpinIdOct, C.int(PIN), &clientSecretOct)
	errorCode = int(rtn)
...
func Client_BN254(hashType, epochDate int, mpinId []byte, RNG *Rand, x []byte, PIN int, token []byte, timePermit []byte, message []byte, epochTime int) (errorCode int, xOut, y, V, U, UT []byte) {
	...
	if RNG == nil {
		...
		pRNG = RNG.csprng()
		xOct = GetOctetZero(PGS_BN254)
	}
	if epochDate == 0 {
		UTOct = nil
		timePermitOct = nil
	} else {
		pUT = GetOctetZero(G1S_BN254)
		UTOct = &pUT
	...
	VOct := GetOctetZero(G1S_BN254)
	defer OctetFree(&VOct)
	UOct := GetOctetZero(G1S_BN254)
	defer OctetFree(&UOct)

	yOct := GetOctetZero(PGS_BN254)
	defer OctetFree(&yOct)

	rtn := C.MPIN_BN254_CLIENT(C.int(hashType), C.int(epochDate), &mpinIdOct, pRNG, &xOct, C.int(PIN), &tokenOct, &VOct, &UOct, UTOct, timePermitOct, messageOct, C.int(epochTime), &yOct)
	errorCode = int(rtn)
...
Precompute_BN254 calculate values for use by the client side of M-Pin Full
...
*/
func Precompute_BN254(token, hashMPinId []byte) (errorCode int, pc1, pc2 []byte) {
	// Form Octets
	hashMPinIdStr := string(hashMPinId)
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
	...
	pc1Oct := GetOctetZero(GTS_BN254)
	defer OctetFree(&pc1Oct)
	pc2Oct := GetOctetZero(GTS_BN254)
	defer OctetFree(&pc2Oct)

	rtn := C.MPIN_BN254_PRECOMPUTE(&tokenOct, &hashMPinIdOct, nil, &pc1Oct, &pc2Oct)
	errorCode = int(rtn)
...
GetG1Multiple_BN254 calculates W=x*P where random x < q is the order of the group of points on the curve.
...
func GetG1Multiple_BN254(RNG *Rand, typ int, x []byte, G []byte) (errorCode int, xOut, W []byte) {
	...
	rtn := C.MPIN_BN254_GET_G1_MULTIPLE(pRNG, C.int(typ), &xOct, &GOct, &WOct)
	errorCode = int(rtn)
...
Server_BN254 performs server side of the one-Pass version of the M-Pin protocol
...
Args:
    ...
    message: message to be signed
    Kangaroo_BN254: Set to true to perform Kangaroo
...
*/
func Server_BN254(hashType, epochDate, epochTime int, serverSecret, U, UT, V, mpinId, publicKey, message []byte, Kangaroo bool) (errorCode int, HID, HTID, y, E, F []byte) {
	...
	HIDOct := GetOctetZero(G1S_BN254)
	defer OctetFree(&HIDOct)
	HTIDOct := GetOctetZero(G1S_BN254)
	...
	if !Kangaroo {
		...
		pE = GetOctetZero(GTS_BN254)
		EOct = &pE
		pF = GetOctetZero(GTS_BN254)
		FOct = &pF
	}

	rtn := C.MPIN_BN254_SERVER(C.int(hashType), C.int(epochDate), &HIDOct, &HTIDOct, &yOct, &serverSecretOct, &UOct, UTOct, &VOct, EOct, FOct, &mpinIdOct, messageOct, C.int(epochTime), publicKeyOct)
	errorCode = int(rtn)
...
Kangaroo_BN254 uses Pollards Kangaroos to find PIN error
...
*/
func Kangaroo_BN254(E []byte, F []byte) (PINError int) {
	EStr := string(E)
	EOct := GetOctet(EStr)
	...
	rtn := C.MPIN_BN254_KANGAROO(&EOct, &FOct)
	PINError = int(rtn)
...
ServerKey_BN254 calculates AES key on server side for M-Pin Full.Uses UT internally for the
...
*/
func ServerKey_BN254(hashType int, Z, serverSecret, w, HM, HID, U, UT []byte) (errorCode int, serverAESKey []byte) {
	var rtn C.int
	ZStr := string(Z)
	ZOct := GetOctet(ZStr)
	defer OctetFree(&ZOct)
	...
	defer OctetFree(&HIDOct)
	serverAESKeyOct := GetOctetZero(PAS_BN254)
	...
	if UT == nil {
		...
		UOct := GetOctet(UStr)
		defer OctetFree(&UOct)
		rtn = C.MPIN_BN254_SERVER_KEY(C.int(hashType), &ZOct, &serverSecretOct, &wOct, &HMOct, &HIDOct, &UOct, nil, &serverAESKeyOct)
	} else {
		UTStr := string(UT)
		UTOct := GetOctet(UTStr)
		defer OctetFree(&UTOct)
		rtn = C.MPIN_BN254_SERVER_KEY(C.int(hashType), &ZOct, &serverSecretOct, &wOct, &HMOct, &HIDOct, nil, &UTOct, &serverAESKeyOct)
	}
...
ClientKey_BN254 calculate AES key on client side for M-Pin Full
...
*/
func ClientKey_BN254(hashType, PIN int, pc1, pc2, r, x, HM, T []byte) (errorCode int, clientAESKey []byte) {
	pc1Str := string(pc1)
	...
	clientAESKeyOct := GetOctetZero(PAS_BN254)
	defer OctetFree(&clientAESKeyOct)

	rtn := C.MPIN_BN254_CLIENT_KEY(C.int(hashType), &pc1Oct, &pc2Oct, C.int(PIN), &rOct, &xOct, &HMOct, &TOct, &clientAESKeyOct)
	errorCode = int(rtn)
...
Server1_BN254 performs first Pass of the server side of the 3-Pass version of the M-Pin protocol
...
*/
func Server1_BN254(hashType, epochDate int, mpinId []byte) (HID, HTID []byte) {
	// Form Octets
	mpinIdStr := string(mpinId)
	mpinIdOct := GetOctet(mpinIdStr)
	defer OctetFree(&mpinIdOct)
	...
	C.MPIN_BN254_SERVER_1(C.int(hashType), C.int(epochDate), &mpinIdOct, &HIDOct, &HTIDOct)
...
Server2_BN254 performs server side of the three-Pass version of the M-Pin protocol, wiht
...
Args:
    ...
    UT: UT = x.(H(ID)+H(epoch_date|H(ID)))
    V: V = -(x+y)(CS+TP), where CS is the reconstructed client secret and TP is the time permit
    Kangaroo_BN254: Set to true to perform Kangaroo_BN254

Returns:

    errorCode: error from the C function
    E: value to help the Kangaroo_BN254s to find the PIN error, or nil Kangaroo_BN254 is set to false
    F: value to help the Kangaroo_BN254s to find the PIN error, or nil Kangaroo_BN254 is set to false

*/
func Server2_BN254(epochDate int, HID []byte, HTID []byte, publicKey []byte, y []byte, serverSecret []byte, U []byte, UT []byte, V []byte, Kangaroo_BN254 bool) (errorCode int, E []byte, F []byte) {
	// Form Octets
	HIDStr := string(HID)
	HIDOct := GetOctet(HIDStr)
	defer OctetFree(&HIDOct)
	...
	rtn := C.MPIN_BN254_SERVER_2(C.int(epochDate), &HIDOct, &HTIDOct, &yOct, &serverSecretOct, &UOct, UTOct, &VOct, EOct, FOct, publicKeyOct)
...
Client1_BN254 performs first Pass of the client side of the three Pass version of the M-Pin protocol.
...
func Client1_BN254(hashType, epochDate int, mpinId []byte, RNG *Rand, x []byte, PIN int, token []byte, timePermit []byte) (errorCode int, xOut, SEC, U, UT []byte) {
	...
	rtn := C.MPIN_BN254_CLIENT_1(C.int(hashType), C.int(epochDate), &mpinIdOct, pRNG, &xOct, C.int(PIN), &tokenOct, &SECOct, &UOct, UTOct, timePermitOct)
...
Client2_BN254 performs second Pass of the client side of the 3-Pass version of the M-Pin protocol
...
*/
func Client2_BN254(x []byte, y []byte, SEC []byte) (errorCode int, V []byte) {
	// Form Octets
	xStr := string(x)
	xOct := GetOctet(xStr)
	defer OctetFree(&xOct)
	...
	rtn := C.MPIN_BN254_CLIENT_2(&xOct, &yOct, &SECOct)
