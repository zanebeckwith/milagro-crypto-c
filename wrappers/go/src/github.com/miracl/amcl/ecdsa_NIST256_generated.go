...
func ECPKeyPairGenerate_NIST256(RNG *Rand, S []byte) (int, []byte, []byte) {
...
func ECPSpDsa_NIST256(hashType int, RNG *Rand, K []byte, S []byte, M []byte) (errorCode int, C []byte, D []byte) {
	KStr := string(K)
	KOct := GetOctet(KStr)
