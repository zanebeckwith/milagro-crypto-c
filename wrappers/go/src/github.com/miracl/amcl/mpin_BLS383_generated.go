...
// ECC points constant
const PAS_BLS383 int = int(C.MPIN_PAS)
const PGS_BLS383 int = int(C.MPIN_PGS_BLS383)
const PFS_BLS383 int = int(C.MPIN_PFS_BLS383)
const G1S_BLS383 = 2*PFS_BLS383 + 1
const G2S_BLS383 = 4 * PFS_BLS383
const GTS_BLS383 = 12 * PFS_BLS383
...
*/
func RandomGenerate_BLS383(RNG *RandNG) (errorCode int, S []byte) {
	// Form Octet
	SOct := GetOctetZero(PGS_BLS383)
	defer OctetFree(&SOct)

	rtn := C.MPIN_BLS383_RANDOM_GENERATE(RNG.csprng(), &SOct)
	errorCode = int(rtn)
...
*/
func GetServerSecret_BLS383(masterSecret []byte) (errorCode int, serverSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	serverSecretOct := GetOctetZero(G2S_BLS383)
	defer OctetFree(&serverSecretOct)

	rtn := C.MPIN_BLS383_GET_SERVER_SECRET(&masterSecretOct, &serverSecretOct)
	errorCode = int(rtn)
...
*/
func RecombineG1_BLS383(R1 []byte, R2 []byte) (errorCode int, R []byte) {
	// Form Octets
	R1Str := string(R1)
	...
	ROct := GetOctetZero(G1S_BLS383)
	defer OctetFree(&ROct)

	rtn := C.MPIN_BLS383_RECOMBINE_G1(&R1Oct, &R2Oct, &ROct)
	errorCode = int(rtn)
...
*/
func RecombineG2_BLS383(W1 []byte, W2 []byte) (errorCode int, W []byte) {
	// Form Octets
	W1Str := string(W1)
	...
	WOct := GetOctetZero(G2S_BLS383)
	defer OctetFree(&WOct)

	rtn := C.MPIN_BLS383_RECOMBINE_G2(&W1Oct, &W2Oct, &WOct)
	errorCode = int(rtn)
...
*/
func GetClientSecret_BLS383(masterSecret []byte, hashMPinId []byte) (errorCode int, clientSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	...
	clientSecretOct := GetOctetZero(G1S_BLS383)
	defer OctetFree(&clientSecretOct)

	rtn := C.MPIN_BLS383_GET_CLIENT_SECRET(&masterSecretOct, &hashMPinIdOct, &clientSecretOct)
	errorCode = int(rtn)
...
*/
func GetDVSKeyPair_BLS383(RNG *RandNG, z []byte) (errorCode int, zOut []byte, publicKey []byte) {
	// Form octets
	publicKeyOct := GetOctetZero(G2S_BLS383)
	...
	if RNG == nil {
		...
		pRNG = RNG.csprng()
		zOct = GetOctetZero(PGS_BLS383)
	}

	rtn := C.MPIN_BLS383_GET_DVS_KEYPAIR(pRNG, &zOct, &publicKeyOct)
	errorCode = int(rtn)
...
*/
func GetClientPermit_BLS383(hashType, epochDate int, masterSecret, hashMPinId []byte) (errorCode int, timePermit []byte) {
	// Form Octets
	...
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
	timePermitOct := GetOctetZero(G1S_BLS383)
	defer OctetFree(&timePermitOct)

	rtn := C.MPIN_BLS383_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), &masterSecretOct, &hashMPinIdOct, &timePermitOct)
	errorCode = int(rtn)
...
*/
func ExtractPIN_BLS383(hashType int, mpinId []byte, PIN int, clientSecret []byte) (errorCode int, token []byte) {
	// Form Octets
	...
	rtn := C.MPIN_BLS383_EXTRACT_PIN(C.int(hashType), &mpinIdOct, C.int(PIN), &clientSecretOct)
	errorCode = int(rtn)
...
*/
func Client_BLS383(hashType, epochDate int, mpinId []byte, RNG *RandNG, x []byte, PIN int, token []byte, timePermit []byte, message []byte, epochTime int) (errorCode int, xOut, y, V, U, UT []byte) {
...
*/
func Precompute_BLS383(token, hashMPinId []byte) (errorCode int, pc1, pc2 []byte) {
	// Form Octets
	hashMPinIdStr := string(hashMPinId)
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
...
*/
func GetG1Multiple_BLS383(RNG *RandNG, typ int, x []byte, G []byte) (errorCode int, xOut, W []byte) {
	xStr := string(x)
	xOct := GetOctet(xStr)
	defer OctetFree(&xOct)
	GStr := string(G)
...
*/
func Client1_BLS383(hashType, epochDate int, mpinId []byte, RNG *RandNG, x []byte, PIN int, token []byte, timePermit []byte) (errorCode int, xOut, SEC, U, UT []byte) {
	// Form Octets
	mpinIdStr := string(mpinId)
	mpinIdOct := GetOctet(mpinIdStr)
	defer OctetFree(&mpinIdOct)
...
*/
func Client2_BLS383(x []byte, y []byte, SEC []byte) (errorCode int, V []byte) {
	// Form Octets
	xStr := string(x)
	xOct := GetOctet(xStr)
	defer OctetFree(&xOct)
	...
	rtn := C.MPIN_BLS383_CLIENT_2(&xOct, &yOct, &SECOct)
