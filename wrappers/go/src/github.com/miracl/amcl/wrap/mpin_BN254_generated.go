...
func RandomGenerate_BN254(rng *Rand) ([]byte, error) {
	rOct := MakeOctet(PGS_BN254)
	defer rOct.Free()

	rtn := C.MPIN_BN254_RANDOM_GENERATE(rng.csprng(), rOct)
...
func GetServerSecret_BN254(ms []byte) ([]byte, error) {
	...
	ssOct := MakeOctet(G2S_BN254)
	defer ssOct.Free()

	rtn := C.MPIN_BN254_GET_SERVER_SECRET(msOct, ssOct)
...
func GetClientSecret_BN254(masterSecret []byte, hashMPinId []byte) ([]byte, error) {
	...
	csOct := MakeOctet(G1S_BN254)
	defer csOct.Free()

	rtn := C.MPIN_BN254_GET_CLIENT_SECRET(msOct, mpOct, csOct)
...
func GetDVSKeyPair_BN254(rng *Rand, z []byte) (zOut, publicKey []byte, err error) {
	pkOct := MakeOctet(G2S_BN254)
	defer pkOct.Free()

	var rtn C.int
	var zOct *Octet
	if rng != nil {
		zOct = MakeOctet(PGS_BN254)
		defer zOct.Free()

		rtn = C.MPIN_BN254_GET_DVS_KEYPAIR(rng.csprng(), zOct, pkOct)
	} else {
		zOct = NewOctet(z)
		defer zOct.Free()

		rtn = C.MPIN_BN254_GET_DVS_KEYPAIR(nil, zOct, pkOct)
	}
...
func GetClientPermit_BN254(hashType, epochDate int, masterSecret, hashMPinId []byte) ([]byte, error) {
	...
	rtn := C.MPIN_BN254_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), msOct, mpOct, tpOct)
...
func Client2_BN254(x, y, SEC []byte) ([]byte, error) {
	...
	rtn := C.MPIN_BN254_CLIENT_2(xOct, yOct, sOct)
