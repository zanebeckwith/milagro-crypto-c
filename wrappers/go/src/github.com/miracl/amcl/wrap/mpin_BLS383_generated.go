...
func RandomGenerate_BLS383(rng *Rand) ([]byte, error) {
	rOct := MakeOctet(PGS_BLS383)
	defer rOct.Free()

	rtn := C.MPIN_BLS383_RANDOM_GENERATE(rng.csprng(), rOct)
...
func GetServerSecret_BLS383(ms []byte) ([]byte, error) {
	...
	ssOct := MakeOctet(G2S_BLS383)
	defer ssOct.Free()

	rtn := C.MPIN_BLS383_GET_SERVER_SECRET(msOct, ssOct)
...
func GetClientSecret_BLS383(masterSecret []byte, hashMPinId []byte) ([]byte, error ) {
	...
	csOct := MakeOctet(G1S_BLS383)
	defer csOct.Free()

	rtn := C.MPIN_BLS383_GET_CLIENT_SECRET(msOct, mpOct, csOct)
...
func GetDVSKeyPair_BLS383(rng *Rand, z []byte) (zOut, publicKey []byte, err error) {
	pkOct := MakeOctet(G2S_BLS383)
	defer pkOct.Free()

	var rtn C.int
	var zOct *Octet
	if rng != nil {
		zOct = MakeOctet(PGS_BLS383)
		defer zOct.Free()

		rtn = C.MPIN_BLS383_GET_DVS_KEYPAIR(rng.csprng(), zOct, pkOct)
	} else {
		zOct = NewOctet(z)
		defer zOct.Free()

		rtn = C.MPIN_BLS383_GET_DVS_KEYPAIR(nil, zOct, pkOct)
	}
...
func GetClientPermit_BLS383(hashType, epochDate int, masterSecret, hashMPinId []byte) ([]byte, error) {
	...
	rtn := C.MPIN_BLS383_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), msOct, mpOct, tpOct)
...
func Client2_BLS383(x, y, SEC []byte) ([]byte, error) {
	...
	rtn := C.MPIN_BLS383_CLIENT_2(xOct, yOct, sOct)
