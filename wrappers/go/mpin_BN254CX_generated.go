...
// ECC points constant
const PAS_BN254CX int = int(C.MPIN_PAS)
const PGS_BN254CX int = int(C.MPIN_PGS_BN254CX)
const PFS_BN254CX int = int(C.MPIN_PFS_BN254CX)
const G1S_BN254CX = 2*PFS_BN254CX + 1
const G2S_BN254CX = 4 * PFS_BN254CX
const GTS_BN254CX = 12 * PFS_BN254CX
...
*/
func RandomGenerate_BN254CX(RNG *RandNG) (errorCode int, S []byte) {
	// Form Octet
	SOct := GetOctetZero(PGS_BN254CX)
	defer OctetFree(&SOct)

	rtn := C.MPIN_BN254CX_RANDOM_GENERATE(RNG.csprng(), &SOct)
	errorCode = int(rtn)
...
*/
func GetServerSecret_BN254CX(masterSecret []byte) (errorCode int, serverSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	serverSecretOct := GetOctetZero(G2S_BN254CX)
	defer OctetFree(&serverSecretOct)

	rtn := C.MPIN_BN254CX_GET_SERVER_SECRET(&masterSecretOct, &serverSecretOct)
	errorCode = int(rtn)
...
*/
func RecombineG1_BN254CX(R1 []byte, R2 []byte) (errorCode int, R []byte) {
	// Form Octets
	R1Str := string(R1)
...
*/
func RecombineG2_BN254CX(W1 []byte, W2 []byte) (errorCode int, W []byte) {
	// Form Octets
	W1Str := string(W1)
...
*/
func GetClientSecret_BN254CX(masterSecret []byte, hashMPinId []byte) (errorCode int, clientSecret []byte) {
	// Form Octets
	masterSecretStr := string(masterSecret)
	masterSecretOct := GetOctet(masterSecretStr)
	defer OctetFree(&masterSecretOct)
	...
	clientSecretOct := GetOctetZero(G1S_BN254CX)
	defer OctetFree(&clientSecretOct)

	rtn := C.MPIN_BN254CX_GET_CLIENT_SECRET(&masterSecretOct, &hashMPinIdOct, &clientSecretOct)
	errorCode = int(rtn)
...
*/
func GetDVSKeyPair_BN254CX(RNG *RandNG, z []byte) (errorCode int, zOut []byte, publicKey []byte) {
	// Form octets
	publicKeyOct := GetOctetZero(G2S_BN254CX)
	...
	if RNG == nil {
		...
		pRNG = RNG.csprng()
		zOct = GetOctetZero(PGS_BN254CX)
	}

	rtn := C.MPIN_BN254CX_GET_DVS_KEYPAIR(pRNG, &zOct, &publicKeyOct)
	errorCode = int(rtn)
...
*/
func GetClientPermit_BN254CX(hashType, epochDate int, masterSecret, hashMPinId []byte) (errorCode int, timePermit []byte) {
	// Form Octets
	...
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
	timePermitOct := GetOctetZero(G1S_BN254CX)
	defer OctetFree(&timePermitOct)

	rtn := C.MPIN_BN254CX_GET_CLIENT_PERMIT(C.int(hashType), C.int(epochDate), &masterSecretOct, &hashMPinIdOct, &timePermitOct)
	errorCode = int(rtn)
...
*/
func ExtractPIN_BN254CX(hashType int, mpinId []byte, PIN int, clientSecret []byte) (errorCode int, token []byte) {
	// Form Octets
	...
	rtn := C.MPIN_BN254CX_EXTRACT_PIN(C.int(hashType), &mpinIdOct, C.int(PIN), &clientSecretOct)
	errorCode = int(rtn)
...
*/
func Client_BN254CX(hashType, epochDate int, mpinId []byte, RNG *RandNG, x []byte, PIN int, token []byte, timePermit []byte, message []byte, epochTime int) (errorCode int, xOut, y, V, U, UT []byte) {
...
*/
func Precompute_BN254CX(token, hashMPinId []byte) (errorCode int, pc1, pc2 []byte) {
	// Form Octets
	hashMPinIdStr := string(hashMPinId)
	hashMPinIdOct := GetOctet(hashMPinIdStr)
	defer OctetFree(&hashMPinIdOct)
...
*/
func GetG1Multiple_BN254CX(RNG *RandNG, typ int, x []byte, G []byte) (errorCode int, xOut, W []byte) {
	xStr := string(x)
	xOct := GetOctet(xStr)
	defer OctetFree(&xOct)
	GStr := string(G)
...
*/
func Client1_BN254CX(hashType, epochDate int, mpinId []byte, RNG *RandNG, x []byte, PIN int, token []byte, timePermit []byte) (errorCode int, xOut, SEC, U, UT []byte) {
	// Form Octets
	mpinIdStr := string(mpinId)
	mpinIdOct := GetOctet(mpinIdStr)
	defer OctetFree(&mpinIdOct)
...
*/
func Client2_BN254CX(x []byte, y []byte, SEC []byte) (errorCode int, V []byte) {
	// Form Octets
	xStr := string(x)
	xOct := GetOctet(xStr)
	defer OctetFree(&xOct)
	...
	rtn := C.MPIN_BN254CX_CLIENT_2(&xOct, &yOct, &SECOct)
