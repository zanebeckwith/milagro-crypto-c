...
func main() {
	...
	HCID := amcl.HashId(HASH_TYPE_MPIN, PFS_ZZZ, ID)
	...
	fmt.Printf("X: 0x")
	fmt.Printf("%x\n", X[:])
	rtn, XOut, Y1, V, U, _ := amcl.Client_ZZZ(HASH_TYPE_MPIN, date, ID[:], &rng, X[:], PIN2, TOKEN[:], nil, MESSAGE[:], timeValue)
	if rtn != 0 {
	...
	fmt.Printf("W: 0x")
	fmt.Printf("%x\n", W[:])
	rtn, WOut, T := amcl.GetG1Multiple_ZZZ(&rng, 0, W[:], HID[:])
	fmt.Printf("WOut: 0x")
	fmt.Printf("%x\n", WOut[:])
	fmt.Printf("T: 0x")
	fmt.Printf("%x\n", T[:])
	...
	// Hash all values
	HM := amcl.HashAll(HASH_TYPE_MPIN, PFS_ZZZ, HCID[:], U[:], nil, V[:], Y2[:], Z[:], T[:])

	// Destroy HM
	defer amcl.CleanMemory(HM[:])

	rtn, AES_KEY_SERVER := amcl.ServerKey_ZZZ(HASH_TYPE_MPIN, Z[:], SS[:], WOut[:], HM[:], HID[:], U[:], nil)
