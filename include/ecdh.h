 ...
 *
 *	@note Deletes all internal state
 *	@param R is a pointer to a cryptographically secure random number generator
 */
...
/**	@brief Calculate HMAC of message M using key K to create tag of length len in octet tag
 *
 *  Calculate HMAC of message M using key K to create tag of length len in octet tag.
 *
 *  @note IEEE-1363 MAC1 function. Uses SHA256 internally.
 *	@param h is the hash type
 ...
 *
 *	@note IEEE-1363 KDF2 Key Derivation Function. Uses SHA256 internally.
 *	@param h is the hash type
 *	@param Z input octet
 ...
 *
 *	@note PBKDF2 Password Based Key Derivation Function. Uses SHA256 internally.
 *	@param h is the hash type
 ...
 *
 *	@note IEEE-1363 AES_CBC_IV0_ENCRYPT function. Encrypts in CBC mode with a zero IV, padding as necessary to create a full final block.
 *	@param K AES key
 ...
 *  AES encrypts a plaintext to a ciphtertext
 *
 *	@note IEEE-1363 AES_CBC_IV0_DECRYPT function. Decrypts in CBC mode with a zero IV.
 *	@param K AES key
 ...
 *
 *	@param RNG is a pointer to a cryptographically secure random number generator, if null then the private key is provided externally in S otherwise it is generated randomly internally
 ...
 */
extern int  ECP_KEY_PAIR_GENERATE(csprng *RNG,octet *s,octet *W);
 ...
 *
 *	@note IEEE-1363 Diffie-Hellman shared secret calculation
 ...
 *  ECIES Encryption.
 *
 *	@note IEEE-1363 ECIES Encryption
 *	@param h is the hash type
 ...
 *  ECDSA Signature.
 *
 *	@note IEEE-1363 ECDSA Signature
 *	@param h is the hash type
 *	@param R is a pointer to a cryptographically secure random number generator
 *	@param k Ephemeral key. This value is used when R=NULL or else it is generated by the function. Not output when set to NULL.
