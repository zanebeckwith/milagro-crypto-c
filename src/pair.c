...
// Ghamman & Fouotsa Method
	...
	FP12_usqr(&y0,r);
	FP12_pow(&y1,&y0,x);
	BIG_fshr(x,1); FP12_pow(&y2,&y1,x); BIG_fshl(x,1); // x must be even
	FP12_conj(&y3,r);
	FP12_mul(&y1,&y3);
	...
	FP12_conj(&y1,&y1);
	FP12_frob(&y1,&X); FP12_frob(&y1,&X); FP12_frob(&y1,&X); 
	FP12_frob(&y2,&X); FP12_frob(&y2,&X); 
	FP12_mul(&y1,&y2);
	...
	FP12_mul(&y1,&y2);
	FP12_copy(&y2,&y3); FP12_frob(&y2,&X);
	FP12_mul(&y1,&y2);
	FP12_copy(r,&y1);
	FP12_reduce(r);
