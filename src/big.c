...
/* faster pseudo-Karatsuba method */
	...
	for (k=1;k<NLEN;k++)
	{
		s+=d[k]; t=co+s; for (i=k;i>=1+k/2;i--) t+=(dchunk)(a[i]-a[k-i])*(b[k-i]-b[i]); c[k]=(chunk)t&BMASK; co=t>>BASEBITS;
	}
	for (k=NLEN;k<2*NLEN-1;k++)
	{
		s-=d[k-NLEN]; t=co+s; for (i=NLEN-1;i>=1+k/2;i--) t+=(dchunk)(a[i]-a[k-i])*(b[k-i]-b[i]); c[k]=(chunk)t&BMASK; co=t>>BASEBITS;
	}
	c[2*NLEN-1]=(chunk)co;
/*
	for (i=0;i<NLEN;i++)
