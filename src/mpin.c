...
static void mpin_hash(int sha,FP4 *f, ECP *P,octet *w)
{
	...
	int hlen=sha;
	BIG_copy(x,f->a.a); FP_redc(x); BIG_toBytes(&t[0],x);
	BIG_copy(x,f->a.b); FP_redc(x); BIG_toBytes(&t[MODBYTES],x);
	BIG_copy(x,f->b.a); FP_redc(x); BIG_toBytes(&t[2*MODBYTES],x);
	BIG_copy(x,f->b.b); FP_redc(x); BIG_toBytes(&t[3*MODBYTES],x);
	ECP_get(x,y,P);
